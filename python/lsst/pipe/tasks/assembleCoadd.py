...
covers a patch on the sky and corresponds to a single run/visit/exposure of the covered patch. We provide the 
task with a list of coaddTempExps (selectDataList) from which it selects coaddTempExps that cover the 
specified patch (pointed at by dataRef). Each coaddTempExp that goes into a coadd will typically have an 
independent photometric zero-point Therefore, we must scale each coaddTempExp to set it to a common 
photometric zeropoint. By default, each coaddTempExp has backgrounds and hence will require 
config.doMatchBackgrounds=True. When background matching is enabled, the task may be configured to 
automatically select a reference exposure (config.autoReference=True). If this is not done, we require that 
the input dataRef provides access to a coaddTempExp (dataset type coaddName + 'Coadd_tempExp') which is used 
as the reference exposure. The coadd is computed as a mean with optional outlier rejection. Criteria for 
outlier rejection are set in \ref AssembleCoaddConfig. Finally, coaddTempExps can have bad 'NaN' pixels which 
received no input from the source calExps. We interpolate over these bad (NaN) pixels.

  \par Inputs:
    coaddTempExp{tract,patch,filter}: ExposureF
  \par Outputs:
...
<DL>
  ...
  <DD> create and use an image scaler object to scale the photometric zeropoint for each temp exposure</DD>
  <DT>\ref MatchBackgroundsTask_ "MatchBackgroundsTask"</DT>
  <DD> match background in a temp exp to a reference exposure (and select the reference exposure if one is 
  not provided).</DD>
  <DT>\ref InterpImageTask_ "InterpImageTask"</DT>
  <DD>interpolate across bad pixels (NaN) in the final coadd</DD>
</DL>
You can replace these subtasks if you wish.
...
the HSC-I & -R band coadds from HSC engineering test data provided in the ci-hsc package. To begin, assuming 
...
\endcode
This will produce warped coaddTempExps for each visit. To coadd the wraped data, we call assembleCoadd.py as 
follows:
\code
...
\ref SafeClipAssembleCoaddTask_ "SafeClipAssembleCoaddTask" rather than AssembleCoaddTask to make the coadd.
    ...
    def run(self, dataRef, selectDataList=[]):
        ...
        Get the skymap, information about the tract & patch to be coadded, the wcs & the outer 
        bbox of the patch specified by the dataRef using \ref getSkyInfo. Pick the exposures to coadd for the 
        specified patch using \ref selectExposures & make the list of corresponding coaddTempExps using 
        \ref getTempExpRefList. Compute weights and inputScalers for each coaddTempExp using 
        \ref prepareInputs. If config.doMatchBackground=True, get a background imageScaler object using 
        \ref getBackgroundReferenceScaler and match backgrounds to the reference using 
        \ref backgroundMatching. Assemble the coaddTempExps using \ref assemble. If background matching was 
        performed, write the appropriate background matching metadata using 
        \ref addBackgroundMatchingMetadata. If config.doInterp=True, interpolate over NaNs using the
        \ref InterpImageTask_ "interpImage" subtask. If config.doWrite=True, call \ref writeCoaddOutput to 
        write out the coadd. Return the coadd.
        ...
        \param[in] selectDataList[in]: List of data references to coaddTempExp. Data to be coadded will be
    ...
    def prepareInputs(self, refList):
        ...
        coaddTempExps together, compute a scale factor to normalize the photometric zeropoint and the weight 
        for each coaddTempExp. 

        For each coaddTempExp, use the \ref ScaleZeroPointTask_ "scaleZeroPoint" subtask to compute a 
        imageScaler object that can be used to scale the coaddTempExp photometric zeropoint to that of the 
        reference coaddTempExp. Extract and scale the maskedImage in the coaddTempExp to the common zeropoint 
        using \ref imageScaler.scaleMaskedImage "scaleMaskedImage".
        Define a \ref afwMath.StatisticsControl "StatisticsControl" object to compute the mean of the variance
        plane of the masked image.
        Assign weight equal to inverse of the mean of the variance plane (i.e. weight noisy coaddTempExps
        less). Return the weights and imageScaler objects.
    ...
    def backgroundMatching(self, inputData, refExpDataRef=None, refImageScaler=None):
        ...
        before coaddition. If no reference is provided, the background matcher selects one. Use the 
        \ref MatchBackgroundsTask_ "matchBackgrounds" subtask to attempt to match backgrounds. This subtask 
        returns a struct that contains a background model, the rms of the fit i.e. sqrt(mean(residuals**2)), 
        the MSE of the reference and matched images: mean((refImage - matchedSciImage)**2), and the mean 
        variance of the difference image. For each non-reference matched background coaddTempExp, if a 
        background model exists for that exposure (i.e. all the other statistics were computed succesfully), 
        compute the ratio of the MSE of the reference and matched images to the mean variance of the 
        difference image. This ratio should be no greater than config.maxMatchResidualRatio (default = 1.1). 
        Also ensure that the fit rms is no greater than config.maxMatchResidualRMS (default = 1.0). If both 
        conditions are met, compute a new weight using new_weight = 1/(1/weight + fitRMS**2) and return a new 
        inputData struct that can be used in place of the input struct.
    ...
    def assemble(self, skyInfo, tempExpRefList, imageScalerList, weightList, bgInfoList=None, 
        ...
        conserve memory usage. Define a \ref afwMath.StatisticsControl "StatisticsControl" object to use when
        performing the coaddition.
        This function controls whether outlier rejection will be performed, the sigma level to 
        reject outliers at, and the number of iterations of outlier rejection to be performed. Before 
        assembling the coadd in each sub-rejion, create the coadded exposure, and write coadd assembly 
        meta-data using \ref assembleMetadata. To begin assembling the coadd, create a 
        \ref afwGeom.Extent2I "bbox" object of size config.subregionSize. Iterate over subregions within the
        outer bbox of the patch using \ref assembleSubregion to mean-stack the corresponding subregions from
        the coaddTempExps (with outlier rejection if config.doSigmaClip=True). Finally set edge bits of coadd
        mask based on weight map using \ref coaddUtils.setCoaddEdgeBits.
    ...
    def assembleSubregion(self, coaddExposure, bbox, tempExpRefList, imageScalerList, weightList,
        ...
        For each coaddTempExpo, check for (and swap in) an alternative mask if one is passed. If background 
        matching is enabled, add the background and background variance from each temp exp. Remove mask planes
        listed in config.removeMaskPlanes (default 'CROSSTALK', 'NOT_DEBLENDED'), Finally, mean-stack the 
        actual exposures using \ref afwMath.statisticsStack "statisticsStack" with outlier rejection if 
...
class SafeClipAssembleCoaddTask(AssembleCoaddTask):
    ...
    We implicitly assume that the coaddTempExps were all observed in the same filter band.
    Each input coaddTempExp covers a patch on the sky and corresponds to a single run/visit/exposure of the
    covered patch.
    We provide the task with a list of coaddTempExps (selectDataList) from which it selects coaddTempExps that
    cover the specified patch (pointed at by dataRef).
    Each coaddTempExp that goes into a coadd will typically have an independent photometric zero-point.
    Therefore, we must scale each coaddTempExp to set it to a common photometric zeropoint.
    By default, each coaddTempExp has backgrounds and hence will require config.doMatchBackgrounds=True.
    When background matching is enabled, the task may be configured to automatically select a reference
    exposure (config.autoReference=True).
    If this is not done, we require that the input dataRef provides access to a coaddTempExp (dataset type
    coaddName + 'Coadd_tempExp') which is used as the reference exposure.
    The coadd is computed as a mean. This poses two problems: i. individual pixels rather than sources are
    detected as outliers and discarded from the coadd - hence we can mistake genuine astronomical transients
    for outliers, & ii. when computing the PSF at a given pixel on the coadd, the contribution from PSFs from
    visits with outlier pixels (at that location on the coadd) rejected as outliers cannot be treated
    correctly \ref AssebleCoaddTask_ "AssembleCoaddTask" does not keep track of outliers.
    In this task, we correct for this behavior by creating a new badMaskPlane 'CLIPPED'. We populate this
    plane on the input coaddTempExps and the final coadd where 1. difference imaging suggests that the
    photometry is un-reliable and ii. there is insufficient overlap between the potential non-astronomical
    artifact and a detected source. Such regions will not contribute to the final coadd. Furthermore, any
    routine to determine the coadd PSF can now be cognizant of clipped regions. As in the case of the base
    \ref AssebleCoaddTask_ "AssembleCoaddTask", coaddTempExps can have bad 'NaN' pixels which received no
    input from the source calExps. In the final step of assembling the coadd, we interpolate over these bad
    (NaN) pixels.
    ...
    will generate the HSC-I & -R band coadds from HSC engineering test data provided in the ci-hsc package. To
    ...
    def assemble(self, skyInfo, tempExpRefList, imageScalerList, weightList, bgModelList, *args, **kwargs):
        ...
        Use \ref buildDifferenceImage to build a difference image identfying pixels that have outlier values
        in some individual visits. Add a new mask plane to the difference image titled "CLIPPED". Using
        \ref detectClip, detect clipped regions on the difference image and set "CLIPPED" on the individual
        coaddTempExps for difference image footprints that overlap significantly with the clipped footprints.
        ...
        footprints acrosss the visits in this process. Use \ref detectClipBig to redo the clip detection by
        determining the clipped region from all overlapping footprints from the detected sources in each
        visit - these are big footprints. Bitwise OR the small clipped footprints and the big footprints in a
        new mask. Add a new bad mask plane "CLIPPED". Turn off outlier removal and run
        \ref AssembleCoaddTask.assemble_ "AssembleCoaddTask.assemble" on the coaddTempExps and the mask of 
        small and big clipped footprints that will now be ignored during co-addition. Return the
        coadded exposure.
    ...
    def buildDifferenceImage(self, skyInfo, tempExpRefList, imageScalerList, weightList, bgModelList):
        ...
        Generate a difference image between clipped and unclipped coadds. Use \ref AssembleCoaddTask.assemble
        with doClip=False to generate a coadd with no outlier clipping. Generate another coadd with outlier
        clipping using \ref AssembleCoaddTask.assemble with doClip=True. Compute the difference image by 
        subtracting oulier clipped coadd from the outlier unclipped coadd. Set the PSF of the difference image
        to that of the outlier unclipped coadd and return the difference image.
    ...
    def detectClip(self, exp, tempExpRefList):
        ...
        Use the \ref SourceDetectionTask_ "clipDetection" subtask to detect foorprints in the diff image after
        clearing the "DETECTED" & "DETECTED_NEGATIVE" mask planes of the diff image and smoothing the diff
        image with a Gaussian kernal with the sigma of the PSF. This yeilds sets of positive and negative
        polarity footprints. Merge them into a single set of footprints. 
        For each footprint, determine the total number of pixels in the footprint (nPixel) and iterate over
        all of the coaddTempExp masks. Determine how many pixels should be ignored in the intersection of the
        footprint and the coaddTempExp because they have a mask bit from config.badMaskPlanes set (ignore)
        using \ref countMaskFromFootprint. Also determine how many pixels in the coaddTempExp have either
        "DETECTED" or "DETECTED_NEGATIVE" set in the intersection of the footprint and the coaddTempExp
        (overlapDet) using \ref countMaskFromFootprint. 
        If the coaddTempExp is workable (ignore < overlapDet or nPixel - ignore >
        0.5*nPixel or overlapDet != 0), compute the overlap fraction = overlapDet/(nPixel - ignore). Identify
        footprints that overlap with one or two input coaddTempExps by comparing the computed overlap fraction
        to thresholds set in the config. A different threshold is applied depending on the number overlapping
        visits. If the overlap exceeds the thresholds, the footprint is considered "CLIPPED". Use 
        \ref lsst.afw.detection.setMaskFromFootprint "setMaskFromFootprint" to propagate the "CLIPPED" mask
        plane of the footprint into the coaddTempExps that the footprint overlaps with. Return a struct with
        the clipped footprints, the indices of the coaddTempExps that end up overlapping with the clipped
    ...
    def detectClipBig(self, tempExpClipList, clipFootprints, clipIndices, maskClipValue, maskDetValue):
        ...
        Get the bad pixel bitmask from the mask plane using \ref getBadPixelMask. Iterate over all the 
        coaddTempExp masks returned by \ref detectClip. For each mask, create a
        \ref lsst.afw.detection.FootprintSet "FoorprintSet" from the "DETECTED" (& "DETECTED_NEGATIVE") pixels
        in the difference coadd. Iterate over all the footprints in the supplied list of footprints marked
        "CLIPPED". Create a mask of the "CLIPPED" regions from all the footprints that overlap the current
        coaddTempExp. Iterating over all the "DETECTED" footprints that have more than config.minBigOverlap
        pixels, count the number of good (i.e.e not in the bad pixel bitmask) pixels in the "CLIPPED" mask
        that overlap with the "DETECTED" footprint. Consider all footprints that have more than
        config.minBigOverlap pixels in the overlap to belong to a big footprint. Clear the "CLIPPED" mask and
        set the detected big footprints as "CLIPPED" in the mask. Bitwise OR the clipped mask plane back into
        the coaddTempExp mask and return the list of big footprints.
